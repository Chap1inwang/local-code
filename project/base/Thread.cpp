...
void CurrentThread::cacheTid(){
	if(t_cachedTid==0){
		t_cachedTid=gettid();
		t_tidStringLength=snprintf(t_tidString,sizeof(t_tidString),"%5d",t_cachedTid);//将输出的字符输出到字符串中
	}
}
...
struct ThreadData{
	...
	:func_(func),name_(name),tid_(tid),CountDownLatch_(CountDownLatch){}
